...
def load_usd_plugin():
    try:
        if not cmds.pluginInfo("mayaUsdPlugin", q=True, loaded=True):
            ...
            print("✅ mayaUsdPlugin loaded")
    except Exception as e:
        print(f"❌ mayaUsdPlugin load failed: {e}")
        raise
...
def export_usd(final_usd_path: str):
    ...
    try:
        cmds.file(final_usd_path,
                  ...
                  pr=True,
                  es=True)
        print(f"✅ USD exported directly to: {final_usd_path}")
        return True
    except Exception as e_direct:
        print(f"⚠️ Direct USD export failed: {e_direct}")
    ...
    try:
        ...
        if ok:
            print(f"✅ USD staged at TEMP and copied to: {final_usd_path}")
            return True
        else:
            print(f"❌ USD copy failed: {err}")
    except Exception as e_stage:
        print(f"❌ Staging USD export failed: {e_stage}")
