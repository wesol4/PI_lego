...
def get_supported_flags():
    """Sprawdź jakie flagi obsługuje AbcExport i zwróć jako set"""
    try:
        help_text = cmds.help("AbcExport")
        return set(help_text.split())
    except Exception:
        return set()
...
def get_top_roots_from_selection_or_scene():
    ...
    if not sel:
        ...
        top = [n for n in all_transforms
               if not n.startswith("|__")
               and n not in ("|persp","|top","|front","|side")]
    else:
        top = []
        for n in sel:
            root = n.split("|")[1] if n.startswith("|") else n
            full = "|" + root
            if full not in top:
                top.append(full)
    return top
...
def build_abc_job_args(roots, start_frame, end_frame, step,
    ...
    # write options (dodaj tylko jeśli są wspierane)
    if world_space:   args += ["-worldSpace"]
    if uvs:           args += ["-uvWrite"]
    if colors and "-writeColorSets" in supported: args += ["-writeColorSets"]
    if visibility:    args += ["-writeVisibility"]
    if write_normals and "-writeNormals" in supported: args += ["-writeNormals"]
    if write_creases and "-writeCreases" in supported: args += ["-writeCreases"]
    if write_face_sets and "-writeFaceSets" in supported: args += ["-writeFaceSets"]
...
def export_alembic(final_abc_path, start_frame, end_frame, step):
    ...
    if not roots:
        print("[WARN] Brak niczego do eksportu (nie znaleziono transformów).")
        return False
    ...
    try:
        ...
        print(f"[OK] Alembic exported to: {final_abc_path}")
        return True
    except Exception as e:
